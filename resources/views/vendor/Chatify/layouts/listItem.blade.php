...
<table class="messenger-list-item" data-contact="{{ $user->id }}">
    <tr data-action="0">
        ...
        style="background-image: url('{{ $user->low_qual_img == null ? asset('users/images/person.png') : asset($user->low_qual_img)  }}');">
        </div>
        </td>
        {{-- center side --}}
        <td>
...
<table class="messenger-list-item" data-contact="{{ $user->id }}">
    <tr data-action="0">
        ...
        <td>
            <p data-id="{{ $user->id }}" data-type="user">
                {{ strlen($user->username) > 12 ? trim(substr($user->username,0,15)).'..' : $user->username }}
            </p>
        </td>
    </tr>
</table>
